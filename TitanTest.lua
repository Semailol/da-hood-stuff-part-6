			...
			if i.KeyCode == Enum.KeyCode.Space then
				if workspace:FindFirstChild(LocalPlayer.Name.."StringKOD") == nil then
					if Jumping() == true and not Poter.IsPlaying and not Pose.IsPlaying then
						AddVelocity(LocalPlayer.Character.HumanoidRootPart.CFrame.LookVector*250, LocalPlayer.Character)
						IsJump = false
						Cookies:Stop()
						CD = true
						play(83594590, true, true)
						Slide()
...
local TimeErase, CanCel = "Time erase!", "Time resume!"
...
function Jump(...)
	(...).Jumping:Connect(function()
		if CD == false and not Poter.IsPlaying and not Pose.IsPlaying and LocalPlayer.Name == MyStand then
			...
			AddVelocity(LocalPlayer.Character.HumanoidRootPart.CFrame.UpVector*170, LocalPlayer.Character)
			play(66868084, true, true)
			delay(0, function()
				IsJump = true
				wait(1) 
				if IsJump == true then
